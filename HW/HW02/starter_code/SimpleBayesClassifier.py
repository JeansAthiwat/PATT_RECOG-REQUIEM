...
class SimpleBayesClassifier:
    ...
    def fit_params(self, x, y, n_bins=10):
        ...
        for feat_i in range(x.shape[1]):  # Iterate over each column (feature)
            for x in [x_stay, x_leave]:
                x_col_no_nan = x[~np.isnan(x[feat_i])]
                print(x_col_no_nan)
                hist, bin_edges = np.histogram(x_col_no_nan[:, feat_i], bins=n_bins)
                self.stay_params[feat_i] = (hist, bin_edges)
