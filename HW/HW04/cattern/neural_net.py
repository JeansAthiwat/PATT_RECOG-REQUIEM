...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        #############################################################################
        # fully_conntected_1 is dot product of input and weights of first layer after adding bias and activation function ReLU
        fully_connected_1 = np.maximum(0, X.dot(W1) + b1)
        # fully_conntected_2 is dot product of fully_conntected_1 and weights of second layer after adding bias
        fully_connected_2 = fully_connected_1.dot(W2) + b2
        ...
        # classifier loss.                                                          #
        #############################################################################
        _, C = scores.shape  # N: number of samples, C: number of classes

        # Step 1: Numerical stability fix
        scores -= np.max(scores, axis=1, keepdims=True)

        # Step 2: Compute softmax probabilities
        softmaxed_logits = np.exp(scores) / np.sum(
            np.exp(scores), axis=1, keepdims=True
        )

        # Step 3: Compute cross-entropy loss
        correct_class_probs = softmaxed_logits[
            np.arange(N), y
        ]  # Extract correct class probabilities
        data_loss = -np.sum(np.log(correct_class_probs)) / N

        # Step 4: Compute L2 regularization loss (only for weights, not biases)
        l2_regularization = reg * (np.sum(W1**2) + np.sum(W2**2))
        ...
        dScores = softmaxed_logits
        dScores[range(N), y] -= 1  # Subtract 1 for the correct class
        dScores /= N  # Normalize by number of examples

        # Gradients for W2 and b2
        grads["W2"] = (
            fully_connected_1.T.dot(dScores) + 2 * reg * W2
        )  # Include regularization
        grads["b2"] = np.sum(dScores, axis=0)

        # Backpropagate into the hidden layer
        dHidden = dScores.dot(W2.T)
        dHidden[fully_connected_1 <= 0] = 0  # Backprop through ReLU

        # Gradients for W1 and b1
        grads["W1"] = X.T.dot(dHidden) + 2 * reg * W1  # Include regularization
